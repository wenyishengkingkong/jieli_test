...
#ifdef NET_BUF_LEAK_TEST
static struct __test_buf {
    u8_t used;
    u8_t id;
    u8_t *addr;
} test_buf[10];
...
void show_test_buf()
{
    printf("show_test_buf");
    for (int i = 0; i < 10; i++) {
        if (test_buf[i].used == 1) {
            printf("buftes buffer[%d]:0x%x", test_buf[i].id, test_buf[i].addr);
        }
    }
    printf("-------------");
}
...
#ifdef NET_BUF_LEAK_TEST
    for (int i = 0; i < 10; i++) {
        if (test_buf[i].used == 0) {
            test_buf[i].addr = buf;
            test_buf[i].id = pool->free_count;
            test_buf[i].used = 1;
            break;
        }
    }
...
{
#ifdef NET_BUF_LEAK_TEST
    printf("net_buf_free:0x%x", buf);
#endif
    struct net_buf_pool *pool;

#ifdef NET_BUF_LEAK_TEST
    for (int i = 0; i < 10; i++) {
        if (test_buf[i].addr == buf) {
            test_buf[i].used = 0;
        }
    }
#endif
    ...
    while (buf) {
        ...
        if (pool->free_count < pool->buf_count) {
            pool->free_count++;
#ifdef NET_BUF_LEAK_TEST
            NET_BUF_INFO("1 free free_count=%d, pool_id=%d", pool->free_count, buf->pool_id);
#endif
        } else {
#ifdef NET_BUF_LEAK_TEST
            NET_BUF_ERR("2 free free_count=%d, pool_id=%d", pool->free_count, buf->pool_id);
#endif
        }
...
void net_buf_unref(struct net_buf *buf)
{
    ...
    if (--buf->ref) {
#ifdef NET_BUF_LEAK_TEST
        printf("net_buf_unref err, buf->ref:%d", buf->ref);
#endif
        irq_unlock(key);
        return;
    }
