...
#define USER_ADV_SEND_INTERVAL \
    (bt_mesh_is_provisioned() || config_bt_mesh_is_provisioner ? config_bt_mesh_node_msg_adv_interval : config_bt_mesh_pb_adv_interval)
#define USER_ADV_SEND_DURATION \
    (bt_mesh_is_provisioned() || config_bt_mesh_is_provisioner ? config_bt_mesh_node_msg_adv_duration : config_bt_mesh_pb_adv_duration)
...
{
    /* BT_DBG("mesh_adv_send_end, buf:0x%x", param); */
    struct net_buf *buf = param;
...
static bool adv_send(struct net_buf *buf)
{
    ...
    /* BT_INFO("--func=%s, delay:%d, duration:%d", __FUNCTION__, delay, duration); */
                      ...
                      void *cb_data)
{
    ...
    BT_INFO("mesh_adv_send buf:0x%x, %s", buf, send_busy ? "busy" : "succ");
}
