    ...
    unsigned int *new_ele_addr)
{
    ...
    if (last_ele_addr != FLASH_INVALID_ADDR) {
        duer_flash_read(&ctx->ctx, last_ele_addr, (char *)&data, sizeof(data));
    ...
    duer_payload_write_flash(&ctx->ctx, addr_start, &data_add, payload_string, payload_len, PAGE_SIZE);
...
static inline int duer_flash_get_last_ele(duer_flash_string_context_t *ctx, unsigned int *addr)
{
    int i = 0;
...
{
    duer_flash_data_header flash_data_header = {0, 0, 0};
    ...
    if (ctx->ele_list[0] != FLASH_INVALID_ADDR) {
        ...
        duer_payload_write_flash(&ctx->ctx, ctx->ele_list[0], &flash_data_header, NULL, 0, PAGE_SIZE);
    }

    if (first_ele_addr != FLASH_INVALID_ADDR) {
        duer_flash_read(&ctx->ctx, first_ele_addr, (char *)&flash_data_header, sizeof(flash_data_header));
        if (flash_data_header.magic == FLASH_MAGIC) {
            flash_data_header.magic = 0;
            flash_data_header.len = FLASH_LEN_BITMASK;
            duer_payload_write_flash(&ctx->ctx, first_ele_addr, &flash_data_header, NULL, 0, PAGE_SIZE);
        }
    }
}

void duer_get_all_ele_from_flash(duer_flash_string_context_t *ctx)
{
    duer_flash_data_header data = {0, 0, 0};
    unsigned int addr = 0;
    int i = 0;
    ...
    for (i = 1; i < ctx->max_ele_count; i++) {
        ...
        if (addr >= ctx->ctx.len) { // search around;
            addr = addr - ctx->ctx.len;
    ...
    char *payload_string)
{
    ...
    if (ctx->ele_list[flash_index] + sizeof(data) + data.len > ctx->ctx.len) {
        i = ctx->ele_list[flash_index] + sizeof(data) + data.len - ctx->ctx.len;
        duer_flash_read(&ctx->ctx, ctx->ele_list[flash_index] + sizeof(data), payload_string, i);
        duer_flash_read(&ctx->ctx, 0, payload_string + i, data.len - i);
    } else {
        duer_flash_read(&ctx->ctx, ctx->ele_list[flash_index] + sizeof(data), payload_string, data.len);
    }
