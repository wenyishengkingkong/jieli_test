...
__attribute__((weak)) int http_server_get_header_callback(const char *file_name)
{
    return 0;
}
...
static int sendFile(char *file, void *sock_hdl, int lowRange, int highRange)
{
    ...
    int err = http_server_get_header_callback(file);
    if (err < 0) {
        badRequest(sock_hdl);
        return -1;
    }
    if (err > 0) {
        if (strlen(file) > 0) {
            fileFound(sock_hdl, http_server_cType_ext(file), strlen(file), lowRange, highRange);
            sock_send(sock_hdl, file, strlen(file), 0);
        } else {
            fileFound(sock_hdl, http_server_cType_ext(file), 0, lowRange, highRange);
        }
        return 0;
    }
...
void acceptPetition(struct http_cli_t *cli)
{
...
#ifdef HTTP_MULTIPART_SUPPORT
    char *requestBuffer = (char *)malloc(MAX_HTTP_BUFFER);
    if (NULL == requestBuffer) {
        puts("acceptPetition malloc err!\n");
    }
    ret = httpd_parse_head(&hdr, cli, requestBuffer, MAX_HTTP_BUFFER);
#else
    ...
    if (!strcmp("POST", hdr.metode)) {
        cli->http_con->state = HTTPs_CONN_STATE_BODY_PARSE;
        ret = https_req_body_form(cli, requestBuffer, MAX_HTTP_BUFFER - 1);
        if (ret) {
            goto EXIT;
        }

        https_send_respone(cli, requestBuffer, MAX_HTTP_BUFFER - 1);
    } else
#endif
    {
