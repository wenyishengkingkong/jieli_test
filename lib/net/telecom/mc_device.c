...
#define PROFILE_PATH "mnt/sdfile/EXT_RESERVED/telecom"
...
static int device_read_flash()
{
    ...
    sdfile_reserve_zone_read(device_data, profile_addr, FLASH_BUF_LEN, 0);
...
void device_erase_flash()
{
    ...
    sdfile_reserve_zone_erase(profile_addr, SDFILE_SECTOR_SIZE, 0);
}
...
void device_write_flash()
{
    ...
    *device_crc 	= CRC16((const void *)&device_data[4], (u32) * device_len);
    ...
    sdfile_reserve_zone_erase(profile_addr, SDFILE_SECTOR_SIZE, 0);
    sdfile_reserve_zone_write(device_data, profile_addr, *device_len + 4, 0);
    free(device_data);
    device_data = NULL;
}
...
static int tcp_device_init(char *tcp_host)
{
    ...
    device.local.sin_addr.s_addr = htonl(INADDR_ANY);
