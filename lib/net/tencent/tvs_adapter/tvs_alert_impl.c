...
static void RTC_alarm(alert_info *new_alert)
{
    ...
    if (time_quit) {
        time_quit = 0;
        thread_fork("RTC_alarm", 20, 500, 0, NULL, RTC_alarm_task, NULL);
    }
#endif
}
...
void alarm_rings_stop(int tvs_alert_stop_reason)
{
    ...
    os_taskq_post("alarm_wakeup", 1, tvs_alert_stop_reason);
}
...
static void wakeup_timeout(void *priv)
{
    ...
    os_taskq_post("alarm_wakeup", 1, TVS_ALART_STOP_REASON_TIMEOUT);
}

static void alarm_wakeup(void *priv)
{
    alert_info *alert = (alert_info *)priv;
    int err = 0;
    int msg[32] = {0};
...
static void rings(void *priv)
{
    ...
    time_quit = 1;
    if (!alarm_wakeup_pid) {
        thread_fork("alarm_wakeup", 20, 500, 10, &alarm_wakeup_pid, alarm_wakeup, priv);
    }
