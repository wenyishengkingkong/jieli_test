...
SW_IIC_PLATFORM_DATA_BEGIN(sw_iic1_data)
	.clk_pin = TCFG_IIC1_CLK,
	.dat_pin = TCFG_IIC1_DAT,
	.sw_iic_delay = 50,
SW_IIC_PLATFORM_DATA_END()
#endif
...
SPI2_PLATFORM_DATA_BEGIN(spi2_data)
    ...
    .mode   = SPI_1WIRE_MODE,//单线
#else
    .mode   = SPI_DUAL_MODE,//双线
#endif
    .port   = 'C',
    .attr	= TCFG_SPI_ATTR,//主从机、边缘极性采样
#endif
	//=======SPI接收摄像头数据
#else
	.mode   = SPI_STD_MODE,
...
#if (CAMERA1_GROUP_PORT == ISC_GROUPC)
		.pclk_gpio   = IO_PORTC_08,
#elif (CAMERA1_GROUP_PORT == ISC_GROUPH)
		.pclk_gpio   = IO_PORTH_04,
#endif
		.hsync_gpio  = -1,
		.vsync_gpio  = -1,
		.data_gpio={
...
REGISTER_DEVICES(device_table) = {
	...
	{ "iic2",  &iic_dev_ops, (void *)&hw_iic1_data },
#else
