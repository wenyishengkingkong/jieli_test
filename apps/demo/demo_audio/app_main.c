...
/*任务列表 */     // 1.任务名称 2.优先级 3.任务堆栈大小，以word为单位 4.任务队列消息大小，以word为单位 5.静态堆栈和队列消息内存，填空则采用动态分配 
...
{
    printf("  ---------------------------------------------------------------------- \n");
    printf("  kingkong main key event handler key key action :%d \n",key->action);
    printf("  kingkong main key event handler key value :%d \n",key->value);
    printf("  ---------------------------------------------------------------------- \n");
