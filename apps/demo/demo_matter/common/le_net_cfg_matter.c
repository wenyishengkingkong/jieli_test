...
static void cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    MATTER_OS_INTF *matter_intf = (MATTER_OS_INTF *)get_mattr_os_intf();
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
                ...
                if (matter_intf->ble_event_callback) {
                    event.type = BLE_GAP_EVENT_CONNECT;
                    event.conn_handle = con_handle;
                    matter_intf->ble_event_callback(&event);
                }
                break;
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    if (handle == 0x0003) {
        if (buffer) {
            strcpy(buffer, "MATTER-3840");
            att_value_len = strlen("MATTER-3840");
        }
    }
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    struct ble_gatt_char_context context;
    struct ble_gap_event event;
    MATTER_OS_INTF *matter_intf = (MATTER_OS_INTF *)get_mattr_os_intf();
    ...
    case ATT_CHARACTERISTIC_00000001_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE :
        ...
        if (matter_intf->ble_gatt_access_op_callback) {
            context.op = BLE_GATT_ACCESS_OP_WRITE_CHR;
            context.value = buffer;
            context.len = buffer_size;
            matter_intf->ble_gatt_access_op_callback(handle, handle, &context);
        }
    ...
    case ATT_CHARACTERISTIC_00000003_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE :
        puts("ATT_CHARACTERISTIC_00000003_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE");
        //0x0002 开指示， 0x0000 关指示
        if (buffer[0] == 0x02 && buffer[1] == 0x00) {
            if (matter_intf->ble_event_callback) {
                event.type = BLE_GAP_EVENT_SUBSCRIBE;
                event.conn_handle = connection_handle;
                event.cur_indicate = 1;
                matter_intf->ble_event_callback(&event);
            }
    ...
    case 0x000c:
        if (matter_intf->ble_gatt_access_op_callback) {
            ...
            context.value = buffer;
            context.len = buffer_size;
            context.conn_handle = connection_handle;
            matter_intf->ble_gatt_access_op_callback(handle, handle, &context);
        }
...
int ble_set_dev_name(char *name, int len)
{
    ...
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_COMPLETE_LOCAL_NAME, (void *)name, name_len);
...
int ble_set_adv_data(unsigned char *data, unsigned int len)
{
#if 1
    if (NULL == data || 0 == len) {
        printf("ble_set_adv_data : PARAM ERROR!");
        return -1;
    }
    ...
    char ser_data[] = {0xF6, 0xFF, 0x00, 0x00, 0x0F, 0xF1, 0xFF, 0x00, 0x80, 0x00};
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_FLAGS, 0x06, 1);
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_SERVICE_DATA, ser_data, 10);
...
int ble_set_rsp_data(unsigned char *data, unsigned int len)
{
    if (NULL == data || 0 == len) {
        return -1;
    }
...
void matter_send_data(const unsigned char *data, const unsigned char len)
{
    struct ble_gap_event event;
    MATTER_OS_INTF *matter_intf = (MATTER_OS_INTF *)get_mattr_os_intf();
    app_send_user_data(0x0e, data, len, ATT_OP_AUTO_READ_CCC);
    if (matter_intf->ble_event_callback) {
