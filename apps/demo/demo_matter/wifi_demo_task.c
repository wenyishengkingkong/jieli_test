...
/***********************************************/
static void (*wifiEventHandle)(unsigned int eventId) = NULL;

void wifi_event_handler_register(void (*event_handle)(int32_t eventId))
{
    wifiEventHandle = event_handle;
}
void matter_main(void);
/***********************************************/
...
#define STA_SSID  "WL83"           //配置 STA模式的SSID
...
//    wifi_set_airkiss_key(airkiss_aes_key);        //配置 airkiss_aes_key
    ...
    if (matter_intf->wifi_event_callback) {
        matter_intf->wifi_event_callback(event);
    }
...
void matter_run_main(void *priv)
{
    //初始化c++
    void cpp_run_init(void);
    cpp_run_init();

    extern void app_main_matter();
    app_main_matter();
}

void matter_main(void)
{
    if (thread_fork("matter_run_main", 10, 10 * 1024, 0, NULL, matter_run_main, NULL) != OS_NO_ERR) {
