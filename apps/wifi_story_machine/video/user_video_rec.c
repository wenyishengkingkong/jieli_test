...
#ifndef USER_ISC_STATIC_BUFF_ENABLE
...
int user_video_rec0_open(int dev)
{
    int err = 0;
#ifdef CONFIG_USR_VIDEO_ENABLE
    union video_req req = {0};
    ...
    if (!__this_user->user_video_rec) {
        if (dev == 2 || dev == 3) {
            __this_user->user_video_rec = server_open("video_server", "video2.0");
            req.rec.uvc_id = dev == 2 ? 0 : 1;
        } else if (dev == 1) {
            __this_user->user_video_rec = server_open("video_server", "video1.0");
        } else {
            __this_user->user_video_rec = server_open("video_server", "video0.0");
        }
        if (!__this_user->user_video_rec) {
            goto __exit;
        }
