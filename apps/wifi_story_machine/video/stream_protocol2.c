/******************************************************************************
* File:             stream_protocol2.c
*
* Author:
* Created:          05/05/23
* Description:      第2路图像实时流(H264)协议转发层
*****************************************************************************/
...
static struct strm_ptl_info strm_ptl_info2_ = {0};
#define strm_ptl (&strm_ptl_info2_)


/*********************************************************************************/
static int stream_protocol2_open(const char *path, const char *mode)
{
    strm_ptl->ptl_fd = stream_open(path, mode);
    if (!strm_ptl->ptl_fd) {
        printf("stream_protocol2_open err !!!\n");
        return -1;
    }
    return 0;
}

static int stream_protocol2_write(u8 type, void *buf, u32 len)
{
    u32 *len_ptr = (u32 *)buf;
    if (!strm_ptl->ptl_fd) {
        return 0;
...
static int stream_protocol2_ctrl(u32 cmd, u32 arg)
{
    if (!strm_ptl->ptl_fd) {
        return 0;
    }
...
static int stream_protocol2_close(void)
{
    int ret;
    if (!strm_ptl->ptl_fd) {
        return 0;
...
int stream_packet2_cb(u8 type, u8 *buf, u32 size)
{
    u32 *head = (u32 *)buf;
...
static void stream_protocol2_task(void *p)
{
    int ret;
    int to = 20;
    ...
    ret = stream_protocol2_open(strm_ptl->spath.path, strm_ptl->spath.mode);
    if (ret) {
        printf("stream_protocol2_open err, path : %s \n", strm_ptl->spath.path);
        strm_ptl->err = true;
        goto exit;
    }
    strm_ptl->init = true;
    ...
    while (1) {
        ...
        if (!ret) {
            ...
            if (strm_ptl->vd_use && strm_ptl->vd_len) {
                ret = stream_protocol2_write(H264_TYPE_VIDEO, strm_ptl->video_pkbuff, strm_ptl->vd_len);
                if (!ret) {
                    strm_ptl->err = true;
                    printf("err : video stream_protocol2_write, kill task !!!\n");
                    break;
                }
            }
            ...
            if (strm_ptl->ad_use && strm_ptl->ad_len) {
                ret = stream_protocol2_write(PCM_TYPE_AUDIO, strm_ptl->audio_pkbuff, strm_ptl->ad_len);
                if (!ret) {
                    strm_ptl->err = true;
                    printf("err : audio stream_protocol2_write, kill task !!!\n");
                    break;
                }
            }
...
int stream_protocol2_task_create(char *path, char *mode)
{
    int ret;
    int to = 100;
    if (strm_ptl->init) {
        printf("stream_protocol2_task already open \n");
        return 0;
    }
    ...
    strm_ptl->ad_use = 0;
    strm_ptl->pid = 0;
    ret = thread_fork("stream_protocol2_task", 16, 1024, 0, &strm_ptl->pid, stream_protocol2_task, NULL);
    if (ret) {
        printf("stream_protocol2_task create err !!!\n");
        return -1;
    }
...
int stream_protocol2_task_kill(void)
{
    if (strm_ptl->init) {
        strm_ptl->kill = true;
        os_sem_post(&strm_ptl->sem);
