...
int main(proto_args)
{
    ...
    if (!getpkt(cmd, 4)) {
        goto alldone;    // cmd[] = width, height, pixelformat, hasmouse
    }
    ...
    while (getpkt(cmd, 1)) {
        ...
        case GNETCODE_PIXEL:
            if (!getpkt(cmd, 3)) {
                goto alldone;    // cmd[] = x, y, color
            }
            gdispDrawPixel(cmd[0], cmd[1], cmd[2]);
            break;
        case GNETCODE_FILL:
            if (!getpkt(cmd, 5)) {
                goto alldone;    // cmd[] = x, y, cx, cy, color
            }
            gdispFillArea(cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);
            break;
        case GNETCODE_BLIT:
            if (!getpkt(cmd, 4)) {
                goto alldone;    // cmd[] = x, y, cx, cy		- Followed by cx * cy pixels
            }
        ...
        case GNETCODE_READ:
            if (!getpkt(cmd, 2)) {
                goto alldone;    // cmd[] = x, y				- Response is GNETCODE_READ,color
            }
        ...
        case GNETCODE_SCROLL:
            if (!getpkt(cmd, 5)) {
                goto alldone;    // cmd[] = x, y, cx, cy, lines
            }
        ...
        case GNETCODE_CONTROL:
            if (!getpkt(cmd, 2)) {
                goto alldone;    // cmd[] = what,data		- Response is GNETCODE_CONTROL, 0x0000 (fail) or GNETCODE_CONTROL, 0x0001 (success)
            }
