...
void gwinMove(GHandle gh, gCoord x, gCoord y)
{
    ...
    if (gh->x > gdispGGetWidth(gh->display) - MIN_WIN_WIDTH) {
        gh->x = gdispGGetWidth(gh->display) - MIN_WIN_WIDTH;
    }
    if (gh->y > gdispGGetHeight(gh->display) - MIN_WIN_HEIGHT)	{
        gh->y = gdispGGetHeight(gh->display) - MIN_WIN_HEIGHT;
    }
    if (gh->x + gh->width > gdispGGetWidth(gh->display)) {
        gh->width = gdispGGetWidth(gh->display) - gh->x;
    }
    if (gh->y + gh->height > gdispGGetHeight(gh->display)) {
        gh->height = gdispGGetHeight(gh->display) - gh->y;
    }
    _gwinUpdate(gh);
}
...
void gwinResize(GHandle gh, gCoord width, gCoord height)
{
    gh->width = width;
    gh->height = height;
    if (gh->width < MIN_WIN_WIDTH) {
        gh->width = MIN_WIN_WIDTH;
    }
    if (gh->height < MIN_WIN_HEIGHT) {
        gh->height = MIN_WIN_HEIGHT;
    }
    if (gh->x + gh->width > gdispGGetWidth(gh->display)) {
        gh->width = gdispGGetWidth(gh->display) - gh->x;
    }
