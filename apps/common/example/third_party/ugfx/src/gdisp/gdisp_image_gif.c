    ...
    while (1) {
        ...
        case 0x21:			//'!' - EXTENSION_RECORD_TYPE;
            ...
            case 0xF9:			// EXTENSION - Graphics Control Block
                ...
                case 0x00:
                case 0x04:
                    break;		// Dispose = do nothing
                case 0x08:
                    priv->frame.flags |= GIFL_DISPOSECLEAR;
                    break;		// Dispose = clear
                case 0x0C:
                case 0x10:
                    priv->frame.flags |= GIFL_DISPOSEREST;
                    break;		// Dispose = restore. Value 0x10 is a hack for bad encoders
                default:
...
gdispImageError gdispGImageDraw_GIF(GDisplay *g, gdispImage *img, gCoord x, gCoord y, gCoord cx, gCoord cy, gCoord sx, gCoord sy)
{
    ...
    if (sx >= fx || sy >= fy || sx + cx < priv->frame.x || sy + cy < priv->frame.y) {
        return GDISP_IMAGE_ERR_OK;
    }
    if (sx < priv->frame.x) {
        mx = priv->frame.x - sx;
