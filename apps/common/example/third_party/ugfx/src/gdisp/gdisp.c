...
#endif
    {
        if (g->p.y < g->clipy0 || g->p.y >= g->clipy1) {
            return;
        }
        if (g->p.x < g->clipx0) {
            g->p.x = g->clipx0;
...
#endif
    {
        if (g->p.x < g->clipx0 || g->p.x >= g->clipx1) {
            return;
        }
        if (g->p.y < g->clipy0) {
            g->p.y = g->clipy0;
...
void gdispGSetClip(GDisplay *g, gCoord x, gCoord y, gCoord cx, gCoord cy)
{
...
#if GDISP_HARDWARE_CLIP != GFXON
    {
        ...
        if (cx <= 0 || cy <= 0 || x >= g->g.Width || y >= g->g.Height) {
            x = y = cx = cy = 0;
        }
        g->clipx0 = x;
        g->clipy0 = y;
        g->clipx1 = x + cx;
        if (g->clipx1 > g->g.Width) {
            g->clipx1 = g->g.Width;
        }
        g->clipy1 = y + cy;
        if (g->clipy1 > g->g.Height) {
            g->clipy1 = g->g.Height;
        }
    }
#endif
...
void gdispGDrawArcSectors(GDisplay *g, gCoord x, gCoord y, gCoord radius, gU8 sectors, gColor color)
{
    ...
    do {
        if (sectors & 0x01) {
            g->p.x = x + b;    // Upper right right
            g->p.y = y - a;
            drawpixel_clip(g);
        }
        if (sectors & 0x02) {
            g->p.x = x + a;    // Upper upper right
            g->p.y = y - b;
            drawpixel_clip(g);
        }
        if (sectors & 0x04) {
            g->p.x = x - a;    // Upper upper left
            g->p.y = y - b;
            drawpixel_clip(g);
        }
        if (sectors & 0x08) {
            g->p.x = x - b;    // Upper left  left
            g->p.y = y - a;
            drawpixel_clip(g);
        }
        if (sectors & 0x10) {
            g->p.x = x - b;    // Lower left  left
            g->p.y = y + a;
            drawpixel_clip(g);
        }
        if (sectors & 0x20) {
            g->p.x = x - a;    // Lower lower left
            g->p.y = y + b;
            drawpixel_clip(g);
        }
        if (sectors & 0x40) {
            g->p.x = x + a;    // Lower lower right
            g->p.y = y + b;
            drawpixel_clip(g);
        }
        if (sectors & 0x80) {
            g->p.x = x + b;    // Lower right right
            g->p.y = y + a;
            drawpixel_clip(g);
        }
        if (P < 0) {
    ...
    if (sectors & 0xC0) {
        g->p.x = x + a;    // Lower right
        g->p.y = y + b;
        drawpixel_clip(g);
    }
    if (sectors & 0x03) {
        g->p.x = x + a;    // Upper right
        g->p.y = y - b;
        drawpixel_clip(g);
    }
    if (sectors & 0x30) {
        g->p.x = x - a;    // Lower left
        g->p.y = y + b;
        drawpixel_clip(g);
    }
    if (sectors & 0x0C) {
        g->p.x = x - a;    // Upper left
        g->p.y = y - b;
        drawpixel_clip(g);
    }
...
void gdispGFillArc(GDisplay *g, gCoord x, gCoord y, gCoord radius, gCoord start, gCoord end, gColor color)
{
...
#if GFX_USE_GMISC && GMISC_NEED_FIXEDTRIG
    sxb = radius * ffcos(start);
    sy = NONFIXED(FIXED0_5 - radius * ffsin(start));
    exb = radius * ffcos(end);
    ey = NONFIXED(FIXED0_5 - radius * ffsin(end));
#elif GFX_USE_GMISC && GMISC_NEED_FASTTRIG
    sxb = FP2FIXED(radius * fcos(start));
    sy = floor(0.5 - radius * fsin(start));
    exb = FP2FIXED(radius * fcos(end));
    ey = floor(0.5 - radius * fsin(end));
#else
    sxb = FP2FIXED(radius * cos(start * GFX_PI / 180));
    sy = floor(0.5 - radius * sin(start * GFX_PI / 180));
    exb = FP2FIXED(radius * cos(end * GFX_PI / 180));
    ey = floor(0.5 - radius * sin(end * GFX_PI / 180));
#endif
    sxd = sy ? sxb / sy : sxb;
    exd = ey ? exb / ey : exb;
    ...
    qtr = 0;
    if (sxb > 0)	{
        qtr |= 0x01;    // S1=0001(1), S2=0000(0), S3=0010(2), S4=0011(3)
    }
    if (sy > 0) {
        qtr |= 0x02;
    }
    if (exb > 0)	{
        qtr |= 0x04;    // E1=0100(4), E2=0000(0), E3=1000(8), E4=1100(12)
    }
    if (ey > 0) {
        qtr |= 0x08;
    }
    if (sy > ey || (sy == ey && sxb > 0))	{
        qtr |= 0x10;    // order of start and end lines
    }
...
void gdispGVerticalScroll(GDisplay *g, gCoord x, gCoord y, gCoord cx, gCoord cy, int lines, gColor bgcolor)
{
...
#endif
    {
        ...
        if (cx <= 0 || cy <= 0 || x >= g->clipx1 || y >= g->clipy1) {
            MUTEX_EXIT(g);
            return;
        }
        if (x + cx > g->clipx1)	{
...
void gdispGDrawBox(GDisplay *g, gCoord x, gCoord y, gCoord cx, gCoord cy, gColor color)
{
    ...
    cx = x + cx - 1;
    cy = y + cy - 1;			// cx, cy are now the end point.
...
void gdispGDrawPoly(GDisplay *g, gCoord tx, gCoord ty, const gPoint *pntarray, unsigned cnt, gColor color)
{
    ...
    g->p.x = tx + p->x;
    g->p.y = ty + p->y;
    g->p.x1 = tx + pntarray->x;
    g->p.y1 = ty + pntarray->y;
    line_clip(g);
...
gCoord gdispGetFontMetric(gFont font, gFontmetric metric)
{
    ...
    case gFontHeight:
        return font->height;
    case gFontDescendersHeight:
        return font->height - font->baseline_y;
    case gFontLineSpacing:
        return font->line_height;
    case gFontCharPadding:
        return 0;
    case gFontMinWidth:
        return font->min_x_advance;
    case gFontMaxWidth:
        return font->max_x_advance;
    case gFontBaselineX:
        return font->baseline_x;
    case gFontBaselineY:
        return font->baseline_y;
    }
    return 0;
}
