...
)
{
    ...
    while (ndata) {	/* Process all tables in the segment */
        if (ndata < 65) {
            return GDISP_IMAGE_ERR_BADDATA;    /* Err: table size is unaligned */
        }
        ndata -= 65;
        d = *data++;							/* Get table property */
        if (d & 0xF0) {
            return GDISP_IMAGE_ERR_BADDATA;    /* Err: not 8-bit resolution */
        }
        i = d & 3;								/* Get table ID */
        pb = alloc_pool(jd, 64 * sizeof(gI32)); /* Allocate a memory block for the table */
        if (!pb) {
            return GDISP_IMAGE_ERR_NOMEMORY;    /* Err: not enough memory */
        }
...
)
{
    ...
    while (ndata) {	/* Process all tables in the segment */
        if (ndata < 17) {
            return GDISP_IMAGE_ERR_BADDATA;    /* Err: wrong data size */
        }
        ndata -= 17;
        ...
        if (d & 0xEE) {
            return GDISP_IMAGE_ERR_BADDATA;    /* Err: invalid class/number */
        }
    ...
    do {
        if (!msk) {				/* Next byte? */
            if (!dc) {			/* No input data is available, re-fill input buffer */
                ...
                if (!dc) {
                    return 0 - (int)GDISP_IMAGE_ERR_BADDATA;    /* Err: read error or wrong stream termination */
                }
            } else {
                dp++;			/* Next data ptr */
            }
    ...
    for (blk = 0; blk < nby + nbc; blk++) {
        ...
        do {
            ...
            if (z) {
                ...
                if (i >= 64) {
                    return GDISP_IMAGE_ERR_BADDATA;    /* Too long zero run */
                }
            }
...
)
{
    ...
    if (JD_USE_SCALE) {
        ...
        ry >>= jd->scale;
        if (!rx || !ry) {
            return GDISP_IMAGE_ERR_OK;    /* Skip this MCU if all pixel is to be rounded off */
        }
        x >>= jd->scale;
        y >>= jd->scale;
    }
...
)
{
    ...
    if (gfileRead(jd->img->f, seg, 2) != 2) {
        return GDISP_IMAGE_ERR_BADDATA;    /* Check SOI marker */
    }
    if (gdispImageGetAlignedBE16(seg, 0) != 0xFFD8) {
        return GDISP_IMAGE_ERR_BADDATA;    /* Err: SOI is not detected */
    }
    ofs = 2;

    for (;;) {
        /* Get a JPEG marker */
        if (gfileRead(jd->img->f, seg, 4) != 4) {
            return GDISP_IMAGE_ERR_BADDATA;
        }
        ...
        case 0xC0:	/* SOF0 (baseline JPEG) */
            ...
            if (gfileRead(jd->img->f, seg, len) != len) {
                return GDISP_IMAGE_ERR_BADDATA;
            }
            ...
            if (seg[5] != 3) {
                return GDISP_IMAGE_ERR_BADDATA;    /* Err: Supports only Y/Cb/Cr format */
            }
            ...
            for (i = 0; i < 3; i++) {
                ...
                if (!i) {	/* Y component */
                    ...
                    if (b != 0x11) {
                        return GDISP_IMAGE_ERR_BADDATA;    /* Err: Sampling factor of Cr/Cb must be 1 */
                    }
                }
                b = seg[8 + 3 * i];							/* Get dequantizer table ID for this component */
                if (b > 3) {
                    return GDISP_IMAGE_ERR_BADDATA;    /* Err: Invalid ID */
                }
                jd->qtid[i] = b;
            }
            break;
        ...
        case 0xDA:	/* SOS */
            ...
            if (!jd->width || !jd->height) {
                return GDISP_IMAGE_ERR_BADDATA;    /* Err: Invalid image size */
            }

            if (seg[0] != 3) {
                return GDISP_IMAGE_ERR_BADDATA;    /* Err: Supports only three color components format */
            }
            ...
            for (i = 0; i < 3; i++) {
                ...
                if (b != 0x00 && b != 0x11)	{
                    return GDISP_IMAGE_ERR_BADDATA;    /* Err: Different table number for DC/AC element */
                }
                b = i ? 1 : 0;
                ...
                if (!jd->qttbl[jd->qtid[i]]) {
                    return GDISP_IMAGE_ERR_BADDATA;    /* Err: Dequantizer table not loaded */
                }
            }
            ...
            if (!n) {
                return GDISP_IMAGE_ERR_BADDATA;    /* Err: SOF0 has not been loaded */
            }
