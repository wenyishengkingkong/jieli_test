...
ret_t application_init()
{
    ...
    return_value_if_fail(thread1 != NULL, RET_BAD_PARAMS);
    tk_thread_set_priority(thread1, TK_THREAD_PRIORITY_NORMAL);
    tk_thread_set_stack_size(thread1, 64 * 1024);
    tk_thread_set_name(thread1, "test_thread1");
    tk_thread_start(thread1);

    thread2 = tk_thread_create(test_thread2, progress_bar2);
    return_value_if_fail(thread2 != NULL, RET_BAD_PARAMS);
    tk_thread_set_priority(thread2, TK_THREAD_PRIORITY_NORMAL);
    tk_thread_set_stack_size(thread2, 64 * 1024);
    tk_thread_set_name(thread2, "test_thread2");
    tk_thread_start(thread2);

    thread3 = tk_thread_create(test_thread3, progress_bar3);
    return_value_if_fail(thread3 != NULL, RET_BAD_PARAMS);
    tk_thread_set_priority(thread3, TK_THREAD_PRIORITY_NORMAL);
    tk_thread_set_stack_size(thread3, 64 * 1024);
    tk_thread_set_name(thread3, "test_thread3");
    tk_thread_start(thread3);

    thread4 = tk_thread_create(test_thread4, progress_bar4);
    return_value_if_fail(thread4 != NULL, RET_BAD_PARAMS);
    tk_thread_set_priority(thread4, TK_THREAD_PRIORITY_NORMAL);
    tk_thread_set_stack_size(thread4, 64 * 1024);
    tk_thread_set_name(thread4, "test_thread4");
