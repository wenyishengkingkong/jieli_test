...
#ifndef HEADER
#define HEADER(x) ((BITMAPINFOHEADER*)(&(x)->bmiHeader))
#endif
#endif
...
static ret_t usb_camera_get_devices_ratio_list_for_device_id(uint32_t device_id,
        slist_t *ratio_list)
{
    ret_t ret = RET_OK;
    HRESULT hr = NOERROR;
...
static ret_t usb_camera_get_devices_ratio_list(void *p_usb_camera_device,
        slist_t *ratio_list)
{
    int piSize = 0;
    int piCount = 0;
                                  ...
                                  uint32_t width, uint32_t height)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        // 优化切换分辨率逻辑
        if (width == 0 || height == 0 || (HEADER(pVih)->biWidth == width && HEADER(pVih)->biHeight == height)) {
            ...
            for (int i = 0; i < VI_NUM_TYPES; i++) {
                if (setSizeAndSubtype(usb_camera_device, pAmMediaType, width, height,
                                      mediaSubtypes[i])) {
                    VIDEOINFOHEADER *new_Vih =
                        reinterpret_cast<VIDEOINFOHEADER *>(pAmMediaType->pbFormat);
                    usb_camera_device->base.device_ratio.width = HEADER(new_Vih)->biWidth;
                    usb_camera_device->base.device_ratio.height = HEADER(new_Vih)->biHeight;
                    usb_camera_device->base.device_ratio.bpp = 3;
                    ret = RET_OK;
                    break;
                }
        ...
        uint32_t data_len)
{
    ...
    color_format_conver(
        ...
        usb_camera_device->base.device_ratio.width,
        usb_camera_device->base.device_ratio.height,
        usb_camera_device->base.device_ratio.width * usb_camera_device->base.device_ratio.bpp,
...
const usb_camera_device_base_vt_t dshow_vt = {
...
extern "C" const usb_camera_drive_info_t dshow_usb_camera_drive_info = {
    "DSHOW",
    &dshow_vt,
};
