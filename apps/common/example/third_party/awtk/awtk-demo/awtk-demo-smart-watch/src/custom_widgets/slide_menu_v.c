...
static ret_t slide_menu_v_do_layout_children(widget_t *widget)
{
    ...
    if (widget_count_children(iter) > 0) {
...
static ret_t slide_menu_v_set_prop(widget_t *widget, const char *name, const value_t *v)
{
    ...
    if (tk_str_eq(name, WIDGET_PROP_VALUE)) {
        ...
        if (v->type == VALUE_TYPE_STRING) {
            if (tk_str_eq(value_str(v), "center")) {
                slide_menu_v_set_align_h(widget, ALIGN_H_CENTER);
            } else if (tk_str_eq(value_str(v), "left")) {
                slide_menu_v_set_align_h(widget, ALIGN_H_LEFT);
            } else if (tk_str_eq(value_str(v), "right")) {
                slide_menu_v_set_align_h(widget, ALIGN_H_RIGHT);
            } else {
                slide_menu_v_set_align_h(widget, ALIGN_H_NONE);
            }
        } else {
