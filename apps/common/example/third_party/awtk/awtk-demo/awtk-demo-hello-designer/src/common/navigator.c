#include "awtk.h"
#include "navigator.h"
extern ret_t animator_init(widget_t *win, void *ctx);
extern ret_t listview_init(widget_t *win, void *ctx);
extern ret_t background_init(widget_t *win, void *ctx);
extern ret_t basic_init(widget_t *win, void *ctx);
...
{
    if (tk_str_eq(name, "animator")) {
        return animator_init(win, ctx);
    } else if (tk_str_eq(name, "listview")) {
        return listview_init(win, ctx);
    } else if (tk_str_eq(name, "background")) {
        return background_init(win, ctx);
    } else if (tk_str_eq(name, "basic")) {
        return basic_init(win, ctx);
