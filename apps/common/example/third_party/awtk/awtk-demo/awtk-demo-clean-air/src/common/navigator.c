...
extern ret_t timing_init(widget_t *win, void *ctx);
extern ret_t setting_init(widget_t *win, void *ctx);
extern ret_t record_init(widget_t *win, void *ctx);
static ret_t navigator_window_init(const char *name, widget_t *win, void *ctx)
{
    if (tk_str_eq(name, "home_page")) {
        ...
        return timing_init(win, ctx);
    } else if (tk_str_eq(name, "setting")) {
        return setting_init(win, ctx);
    } else if (tk_str_eq(name, "record")) {
        return record_init(win, ctx);
    } else {
        return RET_OK;
    }
}
