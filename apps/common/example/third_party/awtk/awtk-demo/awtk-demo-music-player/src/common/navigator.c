#include "awtk.h"
#include "navigator.h"
extern ret_t advanced_init(widget_t *win, void *ctx);
extern ret_t equalizer_init(widget_t *win, void *ctx);
extern ret_t play_list_init(widget_t *win, void *ctx);
...
{
    if (tk_str_eq(name, "advanced")) {
        return advanced_init(win, ctx);
    } else if (tk_str_eq(name, "equalizer")) {
        return equalizer_init(win, ctx);
    } else if (tk_str_eq(name, "play_list")) {
        return play_list_init(win, ctx);
