...
#include "../common/music_manager.h"
#include "../common/player_timer_manager.h"
...
{
    (void)e;
    return navigator_to_with_context("play_list", ctx);
}
...
static ret_t on_btn_equalizer_click(void *ctx, event_t *e)
{
    return navigator_to("equalizer");
}

static ret_t on_btn_advanced_click(void *ctx, event_t *e)
{
    return navigator_to("advanced");
}
...
static ret_t visit_init_child(void *ctx, const void *iter)
{
    ...
    if (name != NULL && *name != '\0') {
        if (tk_str_eq(name, "btn_anim_switch")) {
        ...
        } else if (tk_str_eq(name, "btn_equalizer")) {
            widget_on(widget, EVT_CLICK, on_btn_equalizer_click, win);
        } else if (tk_str_eq(name, "btn_advanced")) {
            widget_on(widget, EVT_CLICK, on_btn_advanced_click, win);
        }
    }
