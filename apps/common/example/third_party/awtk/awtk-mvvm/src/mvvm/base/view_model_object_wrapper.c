...
static ret_t view_model_object_wrapper_on_destroy(tk_object_t *obj)
{
...
static int32_t view_model_object_wrapper_compare(tk_object_t *obj, tk_object_t *other)
{
    return tk_str_cmp(obj->name, other->name);
}

static ret_t view_model_object_wrapper_set_prop(tk_object_t *obj, const char *name,
        const value_t *v)
{
...
static ret_t view_model_object_wrapper_get_prop(tk_object_t *obj, const char *name, value_t *v)
{
    char path[MAX_PATH + 1];
    view_model_object_wrapper_t *object_wrapper = VIEW_MODEL_OBJECT_WRAPPPER(obj);
    return_value_if_fail(obj != NULL && name != NULL && v != NULL, RET_BAD_PARAMS);
...
static bool_t view_model_object_wrapper_can_exec(tk_object_t *obj, const char *name,
        const char *args)
{
    view_model_object_wrapper_t *object_wrapper = VIEW_MODEL_OBJECT_WRAPPPER(obj);
...
static ret_t view_model_object_wrapper_exec(tk_object_t *obj, const char *name, const char *args)
{
    view_model_object_wrapper_t *object_wrapper = VIEW_MODEL_OBJECT_WRAPPPER(obj);
    return_value_if_fail(obj != NULL && name != NULL, RET_BAD_PARAMS);

    return tk_object_exec(TK_OBJECT(object_wrapper->obj), name, args);
}
...
view_model_t *view_model_object_wrapper_create_ex(tk_object_t *obj, const char *prop_prefix)
{
...
view_model_t *view_model_object_wrapper_create(tk_object_t *obj)
{
