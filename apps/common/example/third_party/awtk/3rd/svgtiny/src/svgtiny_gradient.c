        ...
        for (stopnr = 0; stopnr < listlen; ++stopnr) {
            ...
            if (offset != -1 && color != svgtiny_TRANSPARENT) {
                ...
                offset = offset < 0.0f ? 0.0f :
                         offset > 1.0f ? 1.0f :
                         offset < (last_offset + 0.001f) ? (last_offset + 0.001f) :
                         offset;
        ...
        struct svgtiny_parse_state *state)
{
...

    {
        ...
        x0 = gradient_x0 + gradient_dx * state->gradient_stop[0].offset;
        y0 = gradient_y0 + gradient_dy * state->gradient_stop[0].offset;
        x1 = gradient_x0 + gradient_dx * state->gradient_stop[1].offset;
        y1 = gradient_y0 + gradient_dy * state->gradient_stop[1].offset;
        shape->fill_linear_gradient.sx = ctm[0] * x0 + ctm[2] * y0 + ctm[4];
        shape->fill_linear_gradient.sy = ctm[1] * x0 + ctm[3] * y0 + ctm[5];
        shape->fill_linear_gradient.ex = ctm[0] * x1 + ctm[2] * y1 + ctm[4];
        shape->fill_linear_gradient.ey = ctm[1] * x1 + ctm[3] * y1 + ctm[5];
        shape->fill_linear_gradient.icolor = state->gradient_stop[0].color;
        shape->fill_linear_gradient.ocolor = state->gradient_stop[1].color;
