...
static svgtiny_code svgtiny_parse_text(dom_element *text,
                                       struct svgtiny_parse_state state);
static void svgtiny_parse_size_attributes(dom_element *node,
...
static void _svgtiny_parse_color(const char *s, svgtiny_colour *c,
                                 struct svgtiny_parse_state *state);
static void _svgtiny_parse_color_with_gradient(const char *s, svgtiny_colour *c,
        struct svgtiny_parse_state *state);
...
svgtiny_code svgtiny_parse_dom(const char *buffer, size_t size, const char *url, dom_document **output_dom)
{
    ...
    if (!dom_string_caseless_lwc_isequal(svg_name, "svg")) {
...
#undef SVGTINY_STRING_ACTION2
    ...
    svgtiny_parse_size_attributes(svg, state, &width, &height);
    diagram->width = width;
...
void realloc_p(float **p, size_t *p_len, unsigned int i)
{
    if ((*p_len) < i) {
        size_t len = i * 1.2 + 10;
        *p = realloc(*p, sizeof((*p)[0]) * len);
        *p_len = len;
        assert(*p);
    }
}
...
void svgtiny_parse_size_attributes(dom_element *node,
                                   const struct svgtiny_parse_state state,
                                   float *width, float *height)
{
    dom_string *attr;
    ...
    *height = state.viewport_height;
    ...
    exc = dom_element_get_attribute(node, state.interned_viewBox, &attr);
    if (exc == DOM_NO_ERR && attr != NULL) {
        char *s = strndup(dom_string_data(attr),
                          dom_string_byte_length(attr));
        float vx, vy, vwidth, vheight;
        if (sscanf(s, "%f,%f,%f,%f",
                   &vx, &vy, &vwidth, &vheight) == 4 ||
            sscanf(s, "%f %f %f %f",
                   &vx, &vy, &vwidth, &vheight) == 4) {
            *width = vwidth;
            *height = vheight;
                                    ...
                                    struct svgtiny_parse_state *state)
{
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        svgtiny_parse_color_with_gradient(attr, &state->fill, state);
        dom_string_unref(attr);
    }
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        ...
        if ((s = strstr(style, "fill:"))) {
            ...
            _svgtiny_parse_color_with_gradient(value, &state->fill, state);
            free(value);
        }
...
static void __svgtiny_parse_color(const char *s, svgtiny_colour *c,
                                  struct svgtiny_parse_state *state, bool with_gradient)
{
    unsigned int r, g, b;
    float rf, gf, bf;
    size_t len = strlen(s);
    ...
    } else if (len == 11 && strcmp(s, "transparent") == 0) {
        *c = svgtiny_TRANSPARENT;

    } else if (with_gradient && 5 < len && s[0] == 'u' && s[1] == 'r' && s[2] == 'l' &&
               s[3] == '(') {
        if (s[4] == '#') {
            id = strdup(s + 5);
            if (!id) {
...
static void _svgtiny_parse_color(const char *s, svgtiny_colour *c,
                                 struct svgtiny_parse_state *state)
{
    __svgtiny_parse_color(s, c, state, false);
}

static void _svgtiny_parse_color_with_gradient(const char *s, svgtiny_colour *c,
        struct svgtiny_parse_state *state)
{
    __svgtiny_parse_color(s, c, state, true);
}
...
{
    char *ss = strndup(dom_string_data(s), dom_string_byte_length(s));
    _svgtiny_parse_color_with_gradient(ss, c, state);
    free(ss);
}
...
struct svgtiny_shape *svgtiny_add_shape(struct svgtiny_parse_state *state)
{
    ...
    sx = fabsf(state->ctm.a + state->ctm.c);
    sy = fabsf(state->ctm.b + state->ctm.d);
    shape->stroke_width = state->stroke_width * (sx + sy) / 2.0;

    if (shape->stroke_width < 0.001f) {
        shape->stroke_width = 0.0f;
