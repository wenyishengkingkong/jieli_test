...
void iterate_codes(double freqs[], unsigned num, double code_book[], CODEBOOK_TYPE *code_idx)
{
    ...
    do {
        ...
        if (iter_num > 1 && (delta == 0 || fabs(delta_last - delta) / fabs(delta) < 0.000000001)) {
            break;
        }
        delta_last = delta;
    } while (true);
...
bool NGram::save_ngram(FILE *fp)
{
    ...
    if (fwrite(freq_codes_, sizeof(LmaScoreType), kCodeBookSize, fp) != kCodeBookSize) {
        return false;
    }
...
bool NGram::load_ngram(FILE *fp)
{
    ...
    if (fread(freq_codes_, sizeof(LmaScoreType), kCodeBookSize, fp) != kCodeBookSize) {
        return false;
    }
