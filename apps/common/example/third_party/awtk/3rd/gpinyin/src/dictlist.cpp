...
bool DictList::save_list(FILE *fp)
{
    ...
    if (fwrite(start_id_, sizeof(unsigned), kMaxLemmaSize + 1, fp) != kMaxLemmaSize + 1) {
        return false;
    }
...
bool DictList::load_list(FILE *fp)
{
    ...
    if (fread(start_pos_, sizeof(unsigned), kMaxLemmaSize + 1, fp) != kMaxLemmaSize + 1) {
        return false;
    }

    if (fread(start_id_, sizeof(unsigned), kMaxLemmaSize + 1, fp) != kMaxLemmaSize + 1) {
        return false;
    }
