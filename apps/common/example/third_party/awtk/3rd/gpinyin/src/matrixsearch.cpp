...
bool MatrixSearch::alloc_resource()
{
    ...
    if (NULL == dict_trie_ || NULL == spl_parser_ || NULL == share_buf_) {
        return false;
    }
...
{
    if (!inited_ || 0 == pys_decoded_len_ || 0 == matrix_[pys_decoded_len_].mtrx_nd_num) {
        return 0;
    }
...
char16 *MatrixSearch::get_candidate0(char16 *cand_str, unsigned max_len, uint16 *retstr_len,
                                     bool only_unfixed)
{
    if (pys_decoded_len_ == 0 || matrix_[pys_decoded_len_].mtrx_nd_num == 0) {
        return NULL;
    }
