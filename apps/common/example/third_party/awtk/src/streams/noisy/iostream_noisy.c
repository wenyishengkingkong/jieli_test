...
static ret_t tk_iostream_noisy_on_destroy(tk_object_t *obj)
{
    tk_iostream_noisy_t *iostream_noisy = TK_IOSTREAM_NOISY(obj);

    TK_OBJECT_UNREF(iostream_noisy->ostream);
    TK_OBJECT_UNREF(iostream_noisy->real_iostream);
...
tk_iostream_t *tk_iostream_noisy_create(tk_iostream_t *real_iostream)
{
    ...
    obj = tk_object_create(&s_tk_iostream_noisy_vtable);
    iostream_noisy = TK_IOSTREAM_NOISY(obj);
    return_value_if_fail(iostream_noisy != NULL, NULL);

    iostream_noisy->real_iostream = TK_IOSTREAM(tk_object_ref(TK_OBJECT(real_iostream)));
