...
void lv_cache_set_manager(lv_cache_manager_t *manager)
{
    ...
    if (_cache_manager.empty_cb != NULL) {
        _cache_manager.empty_cb();
    } else if (_cache_manager.set_max_size_cb != NULL) {
        _cache_manager.set_max_size_cb(0);
    }
    ...
    if (_cache_manager.set_max_size_cb != NULL) {
        _cache_manager.set_max_size_cb(_cache_manager.max_size);
    }
}
...
{
    LV_ASSERT(_cache_manager.locked);
    if (_cache_manager.find_by_data_cb == NULL) {
        return NULL;
    }
...
{
    LV_ASSERT(_cache_manager.locked);
    if (_cache_manager.find_by_src_cb == NULL) {
        return NULL;
    }
...
{
    LV_ASSERT(_cache_manager.locked);
    if (_cache_manager.invalidate_cb == NULL) {
        return;
    }
...
{
    LV_ASSERT(_cache_manager.locked);
    if (_cache_manager.get_data_cb == NULL) {
        return NULL;
    }
...
{
    LV_ASSERT(_cache_manager.locked);
    if (_cache_manager.release_cb == NULL) {
        return;
    }
