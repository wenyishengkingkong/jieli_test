...
{
    if (entry == NULL) {
        entry = _lv_ll_get_head(&dsc.entry_ll);
    } else {
        entry = _lv_ll_get_next(&dsc.entry_ll, entry);
    }

    while (entry) {
        if (src_type == LV_CACHE_SRC_TYPE_POINTER && entry->src == src) {
            return entry;
        }
        if (src_type == LV_CACHE_SRC_TYPE_PATH && lv_strcmp(entry->src, src) == 0) {
            return entry;
        }
        entry = _lv_ll_get_next(&dsc.entry_ll, entry);
    }
...
static bool drop_youngest(void)
{
    ...
    lv_cache_entry_t *entry = _lv_ll_get_head(&dsc.entry_ll);
    while (entry) {
        if (entry->life < life_min && entry->usage_count == 0) {
            entry_to_drop = entry;
        }
        entry = _lv_ll_get_next(&dsc.entry_ll, entry);
    }
