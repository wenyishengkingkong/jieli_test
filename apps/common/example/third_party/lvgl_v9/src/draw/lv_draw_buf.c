...
void lv_draw_buf_invalidate_cache(void *buf, uint32_t stride, lv_color_format_t color_format, const lv_area_t *area)
{
    if (handlers.invalidate_cache_cb) {
        handlers.invalidate_cache_cb(buf, stride, color_format, area);
    }
}
...
void *lv_draw_buf_go_to_xy(const void *buf, uint32_t stride, lv_color_format_t color_format, int32_t x,
                           int32_t y)
{
    if (handlers.go_to_xy_cb) {
        return handlers.go_to_xy_cb(buf, stride, color_format, x, y);
    } else {
        return NULL;
    }
}
...
{
    if (handlers.buf_clear_cb) {
        handlers.buf_clear_cb(buf, w, h, color_format, a);
    }
}
