    ...
    if (base_dsc->part == LV_PART_ITEMS) {
        ...
        if (base_dsc->id1 == 1) {
            if (draw_task->type == LV_DRAW_TASK_TYPE_FILL) {
                ...
                if (pressed) {
                    rect_draw_dsc->bg_color = lv_palette_darken(LV_PALETTE_BLUE, 3);
                } else {
                    rect_draw_dsc->bg_color = lv_palette_main(LV_PALETTE_BLUE);
                }
        ...
        /*Change the draw descriptor of the 3rd button*/
            ...
            if (draw_task->type == LV_DRAW_TASK_TYPE_FILL) {
                ...
                if (pressed) {
                    rect_draw_dsc->bg_color = lv_palette_darken(LV_PALETTE_RED, 3);
                } else {
