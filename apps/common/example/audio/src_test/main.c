...
void src_test(void)
{
    ...
    int channel = 2;
    //分4种情况,通道数为2作为例子
    //两通道每通道取正弦波数组sindata的len/2个数据，共取len个数据
    for (int situation = 0; situation < 4; situation++) {
        ...
        printf("\n\nsituation = %d\n\r", situation);
        put_buf(bufin, 1024);//打印输入数据

        src_param_t src_parm = {0};
        src_parm.nchannel = channel;

        if (situation > 1) {
            src_parm.in_chinc = len / 2; //顺序输入 每通道数据地址增量,两通道len/2
            src_parm.in_spinc = 1;  //顺序输入 同一通道后一数据相对前一数据的地址增量为1
        } else {
            src_parm.in_chinc = 1; //交织输入 每通道数据地址增量为1
            src_parm.in_spinc = channel; //交织输入 同一通道后一数据相对前一数据的地址增量为channel = 2
        }

        if (situation % 2) {
            src_parm.out_chinc = len / 4; //顺序输出 16k转8k，输出抽值一半，每通道数据地址增量 len/2/2
            src_parm.out_spinc = 1; // 顺序输出 同一通道后一数据相对前一数据的地址增量为1
        } else {
            src_parm.out_chinc = 1; //交织输出 每通道数据地址增量1
            src_parm.out_spinc = channel;//交织输出 同一通道后一数据相对前一数据的地址增量为channel = 2
        }
        ...
        src_parm.in_len = len * 2 + 48; //输入bufin为u16数组 2byte len*2,软件预留48byte滤波器起始操作

        src_parm.out_addr = bufout;
        src_parm.out_len = sizeof(bufout);		//填buf最大值,以输入数据写完为准; 填指定值, 若数据达到该值, 无论输入是否还有数据, 变采样结束,变采样以in_len和out_len其中一个达成数目条件为结束

        printf("-------%s-------%d   inlen = %d  outlen = %d\n\r", __func__, __LINE__, src_parm.in_len, src_parm.out_len);
        src_resample(&src_parm, &src_flt);
        printf("-------%s-------%d   inlen = %d  outlen = %d\n\r", __func__, __LINE__, src_parm.in_len, src_parm.out_len);

        put_buf(bufout, 1024);//打印输出数据
        printf("-----------------------------------------------------------\n\n\n\r");
    }
}
