...
static struct WIFI_SIGN_INFO g_sign_info = {0};
...
static int wifi_event_callback(void *network_ctx, enum WIFI_EVENT event)
{
    ...
    case WIFI_EVENT_MODULE_INIT:
        ...
        if (!strcmp(g_sign_info.str, WIFI_SIGN_STR)) {
            wifi_default_mode_parm.mode = STA_MODE;
            strcpy(sign_ssid, g_sign_info.data.ssid);
            strcpy(sign_pwd, g_sign_info.data.pwd);
        } else {
    ...
    case WIFI_EVENT_STA_START:
        ...
        if (!strcmp(g_sign_info.str, WIFI_SIGN_STR)) {
            SignalingModeSet(1);
            wf_write_xosc(g_sign_info.data.xosc);
            set_pa_config_data(g_sign_info.data.pa);
            wl_anl_actl_init(0);
            for (u8 i = 0; i < sizeof(g_sign_info.data.gain); i++) {
                mp_test_pa_mcs_dgain_set(tx_rate_tab[i].phy, tx_rate_tab[i].mcs, g_sign_info.data.gain[i]);
            }
...
static void fcc_data_deal_task(void *priv)
{
    ...
    for (;;) {
        list_for_each_entry(host, &head, entry) {
            ...
            memset(&g_sign_info, 0, sizeof(struct WIFI_SIGN_INFO));
            syscfg_write(WIFI_SIGN_INDEX, &g_sign_info, sizeof(struct WIFI_SIGN_INFO));
            switch (fcc->opcode) {
            case OP_FCC_INQ_RAND_NUM:
                /* log_info("RAND_NUM = %d\n", RAND_NUM); */
            ...
            case OP_FCC_ENTER_WIFI_SIGN:
                ...
                strcpy(&g_sign_info.str, WIFI_SIGN_STR);
                memcpy(&g_sign_info.data, sign, sizeof(struct WIFI_SIGN));
                syscfg_write(WIFI_SIGN_INDEX, &g_sign_info, sizeof(struct WIFI_SIGN_INFO));
...
u8 rf_fcc_test_init(void)
{
    ...
    if (syscfg_read(WIFI_SIGN_INDEX, &g_sign_info, sizeof(struct WIFI_SIGN_INFO)) == sizeof(struct WIFI_SIGN_INFO)) {
        if (!strcmp(g_sign_info.str, WIFI_SIGN_STR)) {
            log_info("%s, %s\n", __FUNCTION__, g_sign_info.str);
            mode = FCC_WIFI_MODE;
            CUR_MODE = FCC_WIFI_MODE;
            fcc_data_show(OP_FCC_ENTER_WIFI_SIGN, &g_sign_info.data);
            SignalingModeSet(1);
        }
    }
...
u8 fcc_wifi_sign_dgain_set(u8 phy, u8 mcs)
{
    u8 idx;
    if (!strcmp(g_sign_info.str, WIFI_SIGN_STR)) {
        for (idx = 0; idx < ARRAY_SIZE(tx_rate_tab); idx++) {
            if (tx_rate_tab[idx].phy == phy && tx_rate_tab[idx].mcs == mcs) {
                break;
            }
        }

        if (idx >= ARRAY_SIZE(tx_rate_tab)) {
            return FALSE;
        }

        printf("%s, dgain = %d\n", tx_rate_tab[idx].string, g_sign_info.data.gain[idx]);
        mp_test_pa_mcs_dgain_set(phy, mcs, g_sign_info.data.gain[idx]);
        return TRUE;
    }
    return FALSE;
}
