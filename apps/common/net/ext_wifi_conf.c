...
#if ((defined CONFIG_RTL8189E_ENABLE) || (defined CONFIG_RTL8822CS_ENABLE))
...
{
#if ((defined CONFIG_RTL8189E_ENABLE)      || \
	 (defined CONFIG_RTL8822ES_10M_ENABLE) || \
	 (defined CONFIG_RTL8822CS_ENABLE))
    if (enable_wps) {
...
int wifi_get_mac(u8 *mac)
{
    ...
    int ret = netdev_get_mac_addr(mac);
    printf("%s, %s, line = %d, %02x:%02x:%02x:%02x:%02x:%02x\n", __FILE__, __FUNCTION__, __LINE__, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    return ret;
}
#endif
...
{
    return syscfg_read(index, buffer, len);
}


int db_update_buffer(u8 index, char *buffer, int len)
{
    return syscfg_write(index, buffer, len);
}


#ifdef CONFIG_RTL8822CS_ENABLE
...
{
    if (db_select_buffer(EXT_WIFI_MAC_IDX, mac, MAC_ADDR_LEN) == MAC_ADDR_LEN) {
...
char *get_rtw_cache_efuse(void)
{
    ...
    if (get_flash_mac_addr(macaddr) == 0) {
        ...
        macaddr[0] &= ~((1 << 0) | (1 << 1));
        memcpy(&GET_RTW_CACHE_EFUSE_ADDR[MAC_ADDR_OFFSET], macaddr, MAC_ADDR_LEN);
        db_update_buffer(EXT_WIFI_MAC_IDX, macaddr, MAC_ADDR_LEN);
